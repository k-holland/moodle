...
Y.namespace('M.atto_unorderedlist').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    initializer: function() {
        ...
        this.editor.on('keydown', this._shiftEnterHandler, this);
    },

    // If the cursor is in an unordered list, don't allow <shift><enter> keys.
    _shiftEnterHandler: function(e) {
        var inlist = (Y.one(this.get('host').getSelectionParentNode())).ancestor('ul');
        if (inlist && (e.keyCode === 13) && e.shiftKey) {
            e.preventDefault();
        }
    }
});
